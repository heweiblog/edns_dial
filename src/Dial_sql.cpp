...
int open_db( MYSQL *conn,char *host,int port,char *db_name,char *usr,char *pwd)
{
	/*
	unsigned int timeout = 1;
	int ret = mysql_options(conn, MYSQL_OPT_CONNECT_TIMEOUT, (const char *)&timeout);
	if (ret) 
	{
		debug_printf(LOG_LEVEL_ERROR,"set_db:set connect timeout failed,host=%s\n",host);
		return ERROR;
	}
	*/
